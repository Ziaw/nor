...
using BaseClassVisitor = Spark.Nemerle.ChunkVisitors.BaseClassVisitor;
using GeneratedCodeVisitor = Spark.Nemerle.ChunkVisitors.GeneratedCodeVisitor;
using GlobalMembersVisitor = Spark.Nemerle.ChunkVisitors.GlobalMembersVisitor;
...
namespace Spark.Nemerle
{
    public class NemerleViewCompiler : ViewCompiler
    {
        public override void CompileView(IEnumerable<IList<Chunk>> viewTemplates, IEnumerable<IList<Chunk>> allResources)
        {
            ...
            var assembly = batchCompiler.Compile(Debug, "nemerle", SourceCode);
        ...
        public override void GenerateSourceCode(IEnumerable<IList<Chunk>> viewTemplates, IEnumerable<IList<Chunk>> allResources)
        {
            ...
            source.WriteLine("private static _generatedViewId : System.Guid = System.Guid(\"{0:n}\");", GeneratedViewId);
            source.WriteLine("public override GeneratedViewId : System.Guid ");
            source.WriteLine("{ get { _generatedViewId; } }");
            ...
            if (Descriptor != null && Descriptor.Accessors != null)
            {
                foreach (var accessor in Descriptor.Accessors)
                {
                    ...
                    source.Write("{ get { ").Write(accessor.GetValue).WriteLine("; } }");
                }
            }
            ...
            foreach (var viewTemplate in viewTemplates)
            {
                source.WriteLine();
                EditorBrowsableStateNever(source, 4);
                source.WriteLine(string.Format("private RenderViewLevel{0}() : void", renderLevel));
                source.WriteLine("{").AddIndent();
            ...
            source.WriteLine();
            EditorBrowsableStateNever(source, 4); 
            source.WriteLine("public override Render() : void");
            source.WriteLine("{").AddIndent();
